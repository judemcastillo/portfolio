...
export function StickySideNav({ sections }) {
	...
	return (
		<motion.aside
			className="fixed left-0 top-0 hidden h-screen w-48 shrink-0 items-center md:flex z-200"
						...
						return (
							<li key={section.id} className="relative">
								<a
									...
									className={`relative flex items-center gap-4 text-left font-semibold transition-colors h-5 text-md ${
										isActive ? "text-foreground" : "text-muted-foreground"
									}`}
								>
									<span className="relative block h-5 w-5">
										<span className="absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-gray-300 dark:bg-muted" />
										{isActive ? (
											<motion.span
												layoutId="toc-dot"
												className="absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground "
												transition={{
													type: "spring",
													stiffness: 380,
													damping: 28,
